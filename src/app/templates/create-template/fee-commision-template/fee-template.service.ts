...
@Injectable()
export class FeeCommisionTemplateService {
  sharedDate : any;
  ...
  createServiceProfileTemplate(createTemplateRequest : any): Observable<any> {
     ...
     return this.http.post(this.endpoints.E_WALLET_FEE_COMMISION_TEMPLATE_URL, createTemplateRequest);
  }
  ...
  modifyServiceProfileTemplate(createTemplateRequest : any, code : string): Observable<any> {
    return this.http.put(this.endpoints.E_WALLET_FEE_COMMISION_TEMPLATE_URL + '/'+ code, createTemplateRequest);
  ...
  fetchWalletOwnerOnCategory( ): Observable<any> {
    return this.http.get(this.endpoints.E_WALLET_OWNER_URL + '/all?&offset=0&limit=100');
  ...
  getAllDetail(code: string){
    return this.http.get(this.endpoints.E_WALLET_FEE_COMMISION_TEMPLATE_URL + 
      '/allByCriteria?templateCode=' +code ,
      { headers: this.setMastersService.getHeaders() });

  }

  fetchWalletOwnerOnCategoryCode(categoryCode: string): Observable<any> {
...
private dataApprovalRequest(serviceTemplateList : any,templateCategoryName : string,templateCategoryCode :string, featureCode:string,updatedData?: any) {
  let dataApprovalList = [];
      ...
      entity: {feeCommissionTemplateList: serviceTemplateList}
  });
