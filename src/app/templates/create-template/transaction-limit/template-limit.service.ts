...
@Injectable()
export class TemplateLimitService {
  sharedDate : any;
   ...
   */
  createTemplate(createTemplateRequest : any): Observable<any> {
    
  
     return this.http.post(this.endpoints.E_WALLET_TRANSACTION_LIMIT_TEMPLATE_URL, createTemplateRequest);
  }

  modifyServiceProfileTemplate(createTemplateRequest : any, code : string): Observable<any> {
    return this.http.put(this.endpoints.E_WALLET_TRANSACTION_LIMIT_TEMPLATE_URL + '/'+ code, createTemplateRequest);
  ...
  getAllDetail(code: string){
    return this.http.get(this.endpoints.E_WALLET_TRANSACTION_LIMIT_TEMPLATE_URL + 
...
private dataApprovalRequest(serviceTemplateList : any ,templateCategoryName : string,templateCategoryCode :string, featureCode:string,updatedData?: any) {
 console.log('approval TemplateList',serviceTemplateList);
  let dataApprovalList = [];
      ...
      entity: {transactionLimitTemplateList : serviceTemplateList}
  });
