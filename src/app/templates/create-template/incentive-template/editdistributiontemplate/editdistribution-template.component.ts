...
@Component({
  ...
  templateUrl: './editdistribution-template.component.html',
  styleUrls: ['./editdistribution-template.component.css']
})
export class EditDistributionTemplateComponent implements OnInit {
  @Input() data;
  percentComiValue: any;
  ...
  constructor(private commonHelperService: CommonHelperService, private templateService: IncentiveTemplateService,
    private activatedrouter: ActivatedRoute, public activeModal: NgbActiveModal,private router: Router, private approvalService: ApprovalService,private modalService: NgbModal) {
  ...
  async ngOnInit() {
    ...
    this.walletOwnerCategoryName = this.data.walletOwnerCategoryName;
    ...
    if (this.activatedrouter.snapshot.queryParamMap.get('data')) {
      ...
      this.tempcode = this.params.templateCode ==undefined?this.data.templateCode:'';
      this.templateName = this.params.templateName ==undefined?this.data.templateName:'';
      this.channelCode = this.params.channelTypeCode ==undefined?this.data.channelTypeCode:'';
      this.serviceCode = this.params.serviceCode;
      this.serviceCategoryCode = this.params.serviceCategoryCode ==undefined?this.data.serviceCategoryCode:'';
      this.serviceProviderCode = this.params.serviceProviderCode ==undefined?this.data.serviceProviderCode:'';
      ...
      this.textChannelType = this.params.channelTypeName ==undefined?this.data.channelTypeName:'';;
      // this.textTranType = params.transTypeName;
      this.textServiceProvider = this.params.serviceProviderCode ==undefined?this.data.serviceProviderCode:'';
      this.serviceProviderName =this.params.serviceProviderName ==undefined?this.data.serviceProviderName:'';
      this.textServiceCategory = this.params.serviceCategoryName ==undefined?this.data.serviceCategoryName:'';
      this.textService = this.params.serviceName  ==undefined?this.data.serviceName:'';
      ...
      this.taxTypeCode = this.params.taxTypeCode ==undefined?this.data.taxTypeCode:''; //'100000,100001,100002'
      this.percentComiValue=this.params.percentComiValue ==undefined?this.data.percentComiValue:'';
      this.walletOwnerCategoryCode=this.data.walletOwnerCategoryCode;
      // [
      //   {
      //     "code": "100000",
      //     "typeEn": "VAT"
  ...
  public updateTemplate() {
   ...
   // this.serviceTemplateForm.get("toDate").setValue(this.formattedToDate);
    ...
    this.templateService.modifyIncentiveProfileTemplate(this.prepareDataForUpdateStatus(this.serviceTemplateForm.getRawValue()), this.serviceTemplateForm.getRawValue().code).subscribe(result => {
      console.log(result);
      if (result.resultCode === '0') {
       ...
       // this.router.navigate(['../viewDistributionComponent'], { relativeTo: this.activatedrouter, skipLocationChange: true, queryParams: { data: JSON.stringify(params), temp: JSON.stringify(tmp), display: 'block' } });
      }
    });
  ...
  createFormGroup() {
    ...
    this.serviceTemplateForm = new FormGroup({
      ...
      incentiveTemplateCode: new FormControl(this.data.incentiveTemplateCode),
     // templateName: new FormControl(this.templateName),
      ...
      serviceCategoryName: new FormControl(this.textServiceCategory),
     ...
     // fromDate: new FormControl(this.fromDate),
      ...
      fixedComiValue: new FormControl(this.data.fixedComiValue),
