...
@Injectable()
export class OutletBasicInfoService {
   ...
   */
  public createBasicInformationFormGroup(parentWalletCode:string): FormGroup {
    let group = {}
    this.addOuletService.formFields.genInfoKycList.forEach(formField => {
      if (formField.kycMetaDataField === WalletOwnerConstants.kyc.formFieldName.WALLET_OWNER_PARENT) {
        group[formField.kycMetaDataField] = new FormControl(
          // { value: this.addOuletService.walletOwnerParent, disabled: true });
          { value: parentWalletCode, disabled: true });
  ...
  */
  ...
  public prepareCreateWalletOwnerRequest(formData: any): WalletOwnerBasicInfoRequestModel {
    ...
    let createWalletOwnerRequest: WalletOwnerBasicInfoRequestModel = {
      ...formData,
      walletOwnerCategoryCode: this.addOuletService.selectedCategoryCode,
    };
  ...
  public updateWalletOwner(formData: any,wllletOwnerCode:string): Observable<any> {
    return this.http.put<any>(this.endpoints.E_WALLET_OWNER_URL + '/outlet' +'/'+wllletOwnerCode, this.prepareCreateWalletOwnerRequest(formData));
  }
  public createWalletOwner(formData: any): Observable<any> {
    return this.http.post<any>(this.endpoints.E_WALLET_OWNER_URL+'/outlet', this.prepareCreateWalletOwnerRequest(formData));

    // return this.http.post<any>(this.endpoints.E_WALLET_OWNER_URL+'/merchant', this.prepareCreateWalletOwnerRequest(formData));
