...
export class WalletOwnerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.categoryName = WalletOwnerConstants.category.EMPLOYER;
    this.category = WalletOwnerConstants.category.EMPLOYER_CODE; //100009
  ...
  showSuccessMessage() {
    if (this.route.snapshot.queryParamMap.get('status')) {
      if (this.route.snapshot.queryParamMap.get('status').toString() === '1') {
        ...
        //this.successMessage = 'Employer Added successfully, sent for approval';
        this.translate.languageText('WALLET.employerAddedsuccessfullysentforapproval', data=> {
          this.successMessage=data;
        });
        }else{
          //this.successMessage = 'Employer Added successfully';
          this.translate.languageText('WALLET.employerAddedsuccessfully', data=> {
            this.successMessage=data;
          });
        ...
        if(this.walletOwnerService.approvalRequired){
       // this.successMessage = 'Employer Updated successfully , sent for approval';
       this.translate.languageText('WALLET.employerAddedsuccessfullysentforapproval', data=> {
        this.successMessage=data;
      });
        }else{
  ...
  enrolledWalletOwner(walletOwnerCode: number, state: string, stage: string) {
    console.log('--enrolledWalletOwner--', stage);
      this.router.navigate(['enroll', walletOwnerCode], { relativeTo: this.route, queryParams: { stage: stage,code:this.category } });
