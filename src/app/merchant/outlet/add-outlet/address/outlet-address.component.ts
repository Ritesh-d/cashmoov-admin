...
import { OutletAddressService } from './address.service';
...
@Component({
  ...
  styleUrls: ['./outlet-address.component.css']
})
export class OutletAddressComponent implements OnInit {
  ...
  city = WalletOwnerConstants.kyc.formFieldName.CITY;
  ...
  constructor(private addressService: OutletAddressService,private translate : TranslatelanguageService,
  ...
  ngOnInit() {
    this.countryService.countries.subscribe(countriesData => {
      ...
      if (countriesData.resultCode === '0') {
        ...
        this.addressFormFields = this.addOuletService.formFields.addressKycList;
