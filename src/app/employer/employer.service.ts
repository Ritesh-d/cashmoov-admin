...
export class EmployerService {
  ...
  constructor(private router: Router,private route: ActivatedRoute,
    ...
    private endpoints: Endpoints) {
      ...
      //   }
      // });
      this.apporvalRequired(ApprovalConstants.featureCode.EMPLOYER);
  ...
  private walletOwners: EmployerModel[];

  public allWalletOwners(params?): Observable<any> {
    if(params) {
      return this.http.get(this.endpoints.E_WALLET_OWNER_URL+'/employer' + '/all?' + params);
    } else {
      return this.http.get(this.endpoints.E_WALLET_OWNER_URL+'/employer' + '/all?offset=0&limit=30');
    }
  }

  public getWalletOwnerByCode(walletOwnerCode: string): Observable<any> {
    return this.http.get(this.endpoints.E_WALLET_OWNER_URL +'/employer' + '/' + walletOwnerCode);
  }
  ...
  public fetchWalletOwnerOnCategory(categoryCode: string): Observable<any> {
    ...
    //   '/all?walletOwnerCategoryCode=' + categoryCode + '&offset=0&limit=100');
        return this.http.get(this.endpoints.E_WALLET_OWNER_URL +'/employer'+
