...
export class ViewWalletOwnerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.enrollMode = this.commonHelperService.isEnrollMode;
    // console.log('enrollMode'+ this.enrollMode);
      ...
      if (params.id) {
        ...
        this.walletOwnerService.getWalletOwnerByCode(this.walletOwnerCode).subscribe(data => {
          if (data.resultCode === '0') {
            ...
            this.category= WalletOwnerConstants.category.MERCHANT_CODE;
    ...
    if (!this.allAddresses) {
      ...
      if (regionData.resultCode === '0') {
        ...
        this.regions = [{ code: '', type: 'select region' }, ...regionData.regionList];
          ...
          if (data.resultCode === '0' &&  !this.enrollMode) {
    ...
    switch (field) {
      case 'country':
       ...
       this.addresses.controls.forEach(element => {
          this.addresses.controls[i].get('regionCode').enable();
          this.addresses.controls[i].get('regionCode').setValue('');
          i++;
       });
  ...
  onAddAddressForm() {
    (this.allAddresses.get('addresses') as FormArray).push(this.viewWalletOwnerService.blankAddressForm(true));
  }
