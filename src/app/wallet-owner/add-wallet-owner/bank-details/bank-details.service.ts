    ...
    createBankForm() {
        ...
        this.addWalletOwnerService.formFields.bankKycList.forEach(formField => {
            if (formField.kycInstituteType === 'Bank' && formField.kycMetaDataField != 'settlementAccount') {
                // if(formField.kycMetaDataField == "confirmAccountNumber"){
                //     group[formField.kycMetaDataField] = new FormControl('',Validators.pattern('^[0-9]'));
                // }
                // if(formField.kycMetaDataField == "accountNumber"){
                //     group[formField.kycMetaDataField] = new FormControl('',Validators.pattern('^[0-9]'));
                // }else{
